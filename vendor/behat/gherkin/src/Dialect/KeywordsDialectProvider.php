 ...
 */
namespace Behastan202601\Behat\Gherkin\Dialect;
...
final class KeywordsDialectProvider implements DialectProviderInterface
{
    ...
    private function buildDialect(string $language): GherkinDialect
    {
        return new GherkinDialect($language, [
            ...
            'rule' => ['Rule'],
            // Hardcoded value as our old keywords interface doesn't support rules.
    ...
    private static function parseStepKeywords(string $keywordString): array
    {
        ...
        foreach ($legacyKeywords as $legacyKeyword) {
            if (\strlen($legacyKeyword) >= 2 && substr_compare($legacyKeyword, '<', -strlen('<')) === 0) {
                $keyword = (string) substr($legacyKeyword, 0, -1);
                \assert($keyword !== '');
                // phpstan is not smart enough to detect that the length check above guarantees this invariant
                $keywords[] = $keyword;
            } else {
