...
use Behastan202606\Behat\Gherkin\Keywords\KeywordsInterface;
/**
 * Gherkin lexer.
 *
...
class Lexer
{
    ...
    public function analyse($input, $language = 'en')
    {
        ...
        if ('UTF-8' !== mb_detect_encoding($input, 'UTF-8', \true)) {
    ...
    {
        return array('type' => $type, 'line' => $this->lineNumber, 'value' => $value ?: null, 'deferred' => \false);
    }
    /**
        ...
        if (!isset($this->keywordsCache[$type])) {
            ...
            if ('Step' === $type) {
                ...
                foreach (explode('|', $keywords) as $keyword) {
                    $padded[] = \false !== mb_strpos($keyword, '<', 0, 'utf8') ? preg_quote(mb_substr($keyword, 0, -1, 'utf8'), '/') . '\s*' : preg_quote($keyword, '/') . '\s+';
                }
