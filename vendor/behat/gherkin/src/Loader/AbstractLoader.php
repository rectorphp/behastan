...
abstract class AbstractLoader implements LoaderInterface
{
    ...
    public function load($resource)
    {
        if (!$this->supports($resource)) {
            throw new \LogicException(sprintf('%s::%s() was called with unsupported resource `%s`.', static::class, __FUNCTION__, json_encode($resource)));
        }
     ...
     */
    abstract protected function doLoad($resource): array;
}
