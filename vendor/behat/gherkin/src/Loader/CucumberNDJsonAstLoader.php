...
use Behastan202601\Behat\Gherkin\Node\ArgumentInterface;
use Behastan202601\Behat\Gherkin\Node\BackgroundNode;
use Behastan202601\Behat\Gherkin\Node\ExampleTableNode;
use Behastan202601\Behat\Gherkin\Node\FeatureNode;
use Behastan202601\Behat\Gherkin\Node\OutlineNode;
use Behastan202601\Behat\Gherkin\Node\PyStringNode;
use Behastan202601\Behat\Gherkin\Node\ScenarioInterface;
use Behastan202601\Behat\Gherkin\Node\ScenarioNode;
use Behastan202601\Behat\Gherkin\Node\StepNode;
use Behastan202601\Behat\Gherkin\Node\TableNode;
...
class CucumberNDJsonAstLoader extends AbstractLoader
{
    ...
    {
        return array_values(array_filter(array_map(static function ($line) use ($resource) {
            // As we load data from the official Cucumber project, we assume the data matches the JSON schema.
            // @phpstan-ignore argument.type
            return self::getFeature(json_decode($line, \true, 512, 0), $resource);
        }, file($resource) ?: throw new RuntimeException("Could not load Cucumber json file: {$resource}."))));
    }
    /**
    ...
    private static function getFeature(array $json, string $filePath): ?FeatureNode
    {
        ...
        return new FeatureNode($featureJson['name'], $featureJson['description'], self::getTags($featureJson), self::getBackground($featureJson), self::getScenarios($featureJson), $featureJson['keyword'], $featureJson['language'], preg_replace('/(?<=\.feature).*$/', '', $filePath), $featureJson['location']['line']);
    }
    /**
    ...
    {
        return array_map(static function (array $tag) {
            return preg_replace('/^@/', '', $tag['name']) ?? $tag['name'];
        }, $json['tags']);
    }
    /**
    ...
    {
        return array_values(array_map(static function ($child) {
            $tables = self::getTables($child['scenario']['examples']);
            if ($tables) {
                return new OutlineNode($child['scenario']['name'], self::getTags($child['scenario']), self::getSteps($child['scenario']['steps']), $tables, $child['scenario']['keyword'], $child['scenario']['location']['line']);
            }
            return new ScenarioNode($child['scenario']['name'], self::getTags($child['scenario']), self::getSteps($child['scenario']['steps']), $child['scenario']['keyword'], $child['scenario']['location']['line']);
        }, array_filter($json['children'], static function ($child) {
    ...
    {
        $backgrounds = array_filter($json['children'], static function ($child) {
        ...
        return new BackgroundNode($background['background']['name'], self::getSteps($background['background']['steps']), $background['background']['keyword'], $background['background']['location']['line']);
    }
    /**
    ...
    private static function getSteps(array $items): array
    {
        return array_map(static function (array $item) {
            return new StepNode(trim($item['keyword']), $item['text'], self::getStepArguments($item), $item['location']['line'], trim($item['keyword']));
        }, $items);
    }
    /**
    ...
    private static function getStepArguments(array $step): array
    {
        ...
        if (isset($step['docString'])) {
            $args[] = new PyStringNode(explode("\n", $step['docString']['content']), $step['docString']['location']['line']);
        }
    ...
    {
        return array_map(static function ($tableJson): ExampleTableNode {
            $headerRow = $tableJson['tableHeader'] ?? null;
            $tableBody = $tableJson['tableBody'];
            if ($headerRow === null && $tableBody !== []) {
                throw new NodeException(sprintf('Table header is required when a table body is provided for the example on line %s.', $tableJson['location']['line']));
            }
            $table = [];
            if ($headerRow !== null) {
            ...
            return new ExampleTableNode($table, $tableJson['keyword'], self::getTags($tableJson));
        }, $items);
    }
}
