...
use Behastan202601\Behat\Gherkin\Gherkin;
...
class DirectoryLoader extends AbstractFileLoader
{
    ...
    {
        return is_string($resource) && ($path = $this->findAbsolutePath($resource)) !== \false && is_dir($path);
    }
    /**
     * @param mixed $resource
     */
    protected function doLoad($resource): array
    {
        ...
        $paths = array_map(\Closure::fromCallable('strval'), iterator_to_array($iterator));
        uasort($paths, \Closure::fromCallable('strnatcasecmp'));
        $features = [];
