...
final class Filesystem
{
    ...
    {
        try {
            $result = self::callSafely(static function () use ($fileName) {
                return file_get_contents($fileName);
            });
        } catch (ErrorException $e) {
            throw new FilesystemException(sprintf('File "%s" cannot be read: %s', $fileName, $e->getMessage()), 0, $e);
        }
    ...
    public static function writeFile(string $fileName, string $content): void
    {
        ...
        try {
            $result = self::callSafely(static function () use ($fileName, $content) {
                return file_put_contents($fileName, $content);
            });
        } catch (ErrorException $e) {
            throw new FilesystemException(sprintf('File "%s" cannot be written: %s', $fileName, $e->getMessage()), 0, $e);
        }
    ...
    {
        $result = json_decode(self::readFile($fileName), \true, 512, 0);
    ...
    public static function readJsonFileHash(string $fileName): array
    {
        ...
        assert($result === array_filter($result, \Closure::fromCallable('is_string'), \ARRAY_FILTER_USE_KEY), 'File must contain a JSON object at its root');
        return $result;
    }
    /**
     * @return list<string>
    ...
    public static function getLastModified(string $fileName): int
    {
        try {
            ...
            throw new FilesystemException(sprintf('Last modification time of file "%s" cannot be found: %s', $fileName, $e->getMessage()), 0, $e);
        }
    ...
    public static function ensureDirectoryExists(string $path): void
    {
        ...
        try {
            $result = self::callSafely(static function () use ($path) {
                return mkdir($path, 0777, \true);
            });
            ...
            throw new FilesystemException(sprintf('Path at "%s" cannot be created: %s', $path, $e->getMessage()), 0, $e);
        }
    }
    /**
     * @template TResult
     ...
     */
    private static function callSafely(callable $callback)
    {
