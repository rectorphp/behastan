 ...
 */
namespace Behastan202601\Behat\Gherkin\Cache;

use Behastan202601\Behat\Gherkin\Exception\CacheException;
...
use Behastan202601\Composer\InstalledVersions;
/**
 * File cache.
...
class FileCache implements CacheInterface
{
    ...
    public function __construct(string $path)
    {
        ...
        try {
            ...
            throw new CacheException(sprintf('Cache path "%s" cannot be created or is not a directory: %s', $this->path, $ex->getMessage()), 0, $ex);
        }
    ...
    public function read(string $path)
    {
        $cachePath = $this->getCachePathFor($path);
        try {
            $feature = unserialize(Filesystem::readFile($cachePath), ['allowed_classes' => \true]);
        } catch (FilesystemException $ex) {
            throw new CacheException("Can not load cache: {$ex->getMessage()}", 0, $ex);
        }
