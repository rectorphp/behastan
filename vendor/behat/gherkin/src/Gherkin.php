...
use Behastan202601\Behat\Gherkin\Filter\FeatureFilterInterface;
use Behastan202601\Behat\Gherkin\Filter\LineFilter;
use Behastan202601\Behat\Gherkin\Filter\LineRangeFilter;
use Behastan202601\Behat\Gherkin\Loader\FileLoaderInterface;
use Behastan202601\Behat\Gherkin\Loader\LoaderInterface;
...
class Gherkin
{
    ...
    {
        $this->filters = [];
        array_map(\Closure::fromCallable([$this, 'addFilter']), $filters);
    }
    /**
    ...
    public function load($resource, array $filters = [])
    {
        ...
        $matches = [];
        if (is_scalar($resource) || is_object($resource) && method_exists($resource, '__toString')) {
