...
use Behastan202601\Behat\Gherkin\Exception\InvalidTagContentException;
...
use Behastan202601\Behat\Gherkin\Exception\UnexpectedParserNodeException;
use Behastan202601\Behat\Gherkin\Exception\UnexpectedTaggedNodeException;
...
class Parser implements ParserInterface
{
     ...
     */
    private $tags = [];
    public function __construct(Lexer $lexer, GherkinCompatibilityMode $compatibilityMode = GherkinCompatibilityMode::LEGACY)
    {
        $this->lexer = $lexer;
    ...
    public function parseFile(string $file): ?FeatureNode
    {
        try {
            ...
            throw new ParserException("Cannot parse file: {$ex->getMessage()}", 0, $ex);
        }
    }
    /**
    ...
    protected function expectTokenType(string $type)
    {
        ...
        throw new ParserException(sprintf('Expected %s token, but got %s on line: %d%s', $type, $this->predictTokenType(), $token['line'], $this->file ? ' in file: ' . $this->file : ''));
    }
    /**
     * Returns next token if it's type equals to expected.
     *
    ...
    protected function parseFeature()
    {
        ...
        while ($this->predictTokenType() !== 'EOS') {
            ...
            throw new UnexpectedParserNodeException((function () use ($isBackgroundAllowed) {
                switch ($isBackgroundAllowed) {
                    case \true:
                        return 'Background, Scenario or Outline';
                    case \false:
                        return 'Scenario or Outline';
                }
            })(), $node, $this->file);
        }
        return new FeatureNode($title, $description, $tags, $background, $scenarios, $keyword, $language, $file, $line);
    }
    /**
     ...
     * @return \Behat\Gherkin\Node\OutlineNode|\Behat\Gherkin\Node\ScenarioNode
     */
    private function parseScenarioOrOutlineBody(array $token)
    {
        ['title' => $title, 'description' => $description] = $this->parseTitleAndDescription($token);
        $tags = $this->popTags();
        $keyword = $token['keyword'];
        ...
        while (in_array($nextTokenType = $this->predictTokenType(), ['Step', 'Examples', 'Newline', 'Text', 'Comment', 'Tag'])) {
            ...
            throw new UnexpectedParserNodeException((function () use ($examples) {
                switch ($examples) {
                    case []:
                        return 'Step, Examples table, or end of Scenario';
                    default:
                        return 'Examples table or end of Scenario';
                }
            })(), $node, $this->file);
        }
        if ($examples !== []) {
    ...
    private function validateAndGetNextTaggedNodeType(): string
    {
        ...
        try {
            while (\true) {
                ...
                $nextType = $next['type'];
                if (in_array($nextType, ['Tag', 'Comment', 'Newline'], \true)) {
                    // These are the only node types allowed between tag node(s) and the node they are tagging
                    continue;
                }
                if (in_array($nextType, ['Feature', 'Examples', 'Scenario', 'Outline'], \true)) {
    ...
    protected function guardTags(array $tags)
    {
        foreach ($tags as $tag) {
            if (preg_match('/\s/', $tag)) {
                ...
                trigger_error(sprintf('Whitespace in tags is deprecated, found "%s" in %s', $tag, $this->file ?? 'unknown file'), \E_USER_DEPRECATED);
            }
        }
    }
    /**
    ...
    private function parseTitleAndDescription(array $keywordToken): array
    {
        ...
        while (in_array($predicted = $this->predictTokenType(), ['Newline', 'Text', 'Comment'])) {
            ...
            switch ($token['type']) {
                ...
                case 'Text':
                    ...
                    break;
                default:
                    throw new LogicException('Unexpected token type: ' . $token['type']);
            }
        ...
        if ($this->compatibilityMode->allowAllNodeDescriptions()) {
            ...
            return ['title' => $originalTitle ?: null, 'description' => trim(implode("\n", $textLines), "\n") ?: null];
        }
        if ($keywordToken['type'] === 'Feature') {
            ...
            return ['title' => $originalTitle ?: null, 'description' => rtrim(implode("\n", $textLines), "\n") ?: null];
        }
        ...
        return ['title' => rtrim(implode("\n", $textLines)) ?: null, 'description' => null];
    }
    /**
    ...
    private function normalizeStepNodeKeywordType(StepNode $node, array $steps = []): StepNode
    {
        ...
        return new StepNode($node->getKeyword(), $node->getText(), $node->getArguments(), $node->getLine(), $keywordType);
    }
    /**
     * @return never
     */
    private function rethrowNodeException(NodeException $e)
    {
