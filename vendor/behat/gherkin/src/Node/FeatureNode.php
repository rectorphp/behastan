...
class FeatureNode implements KeywordNodeInterface, TaggedNodeInterface, DescribableNodeInterface
{
    ...
    /**
     * @readonly
     * @var \Behat\Gherkin\Node\BackgroundNode|null
     */
    private $background;
    /**
     ...
     */
    public function __construct(?string $title, ?string $description, array $tags, ?BackgroundNode $background, array $scenarios, string $keyword, string $language, ?string $file, int $line)
    {
        $this->title = $title;
    ...
    {
        return new self($this->title, $this->description, $this->tags, $this->background, array_values($scenarios), $this->keyword, $this->language, $this->file, $this->line);
    }
    /**
