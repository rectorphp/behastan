...
class OutlineNode implements ScenarioInterface, DescribableNodeInterface
{
     ...
     */
    public function __construct(?string $title, array $tags, array $steps, $tables, string $keyword, int $line, ?string $description = null)
    {
        $this->title = $title;
        $this->tags = $tags;
        $this->steps = $steps;
    ...
    {
        return new self($this->title, $this->tags, $this->steps, $exampleTables, $this->keyword, $this->line, $this->description);
    }
    /**
    ...
    protected function createExamples()
    {
        ...
        foreach ($this->getExampleTables() as $exampleTable) {
            foreach ($exampleTable->getColumnsHash() as $rowNum => $row) {
                $examples[] = new ExampleNode($exampleTable->getRowAsString($rowNum + 1), array_merge($this->tags, $exampleTable->getTags()), $this->getSteps(), $row, $exampleTable->getRowLine($rowNum + 1), $this->getTitle(), $rowNum + 1);
            }
        }
        return $examples;
    }
