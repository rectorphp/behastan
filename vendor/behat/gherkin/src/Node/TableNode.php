...
class TableNode implements ArgumentInterface, IteratorAggregate
{
    ...
    public function __construct(array $table)
    {
        ...
        foreach ($this->getRows() as $rowIndex => $row) {
            if (!is_array($row)) {
                throw new NodeException(sprintf("Table row '%s' is expected to be array, got %s", $rowIndex, gettype($row)));
            }
            if ($columnCount === null) {
                $columnCount = count($row);
            }
            if (count($row) !== $columnCount) {
                throw new NodeException(sprintf("Table row '%s' is expected to have %s columns, got %s", $rowIndex, $columnCount, count($row)));
            }
            foreach ($row as $columnIndex => $cellValue) {
                ...
                if (!is_scalar($cellValue)) {
                    throw new NodeException(sprintf("Table cell at row '%s', column '%s' is expected to be scalar, got %s", $rowIndex, $columnIndex, get_debug_type($cellValue)));
                }
    ...
    public function getColumnsHash()
    {
        ...
        foreach ($rows as $row) {
            assert($keys !== null);
            // If there is no first row due to an empty table, we won't enter this loop either.
