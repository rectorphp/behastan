...
final class UnusedContextDefinitionsRule implements RuleInterface
{
    ...
    public function process(array $contextFiles, array $featureFiles, PatternCollection $patternCollection, string $projectDirectory): array
    {
        ...
        foreach ($unusedPatterns as $unusedPattern) {
            $errorMessage = sprintf('The pattern "%s" and its definition %s::%s() is never used', $unusedPattern->pattern, $unusedPattern->className, $unusedPattern->methodName);
            $ruleErrors[] = new RuleError($errorMessage, [$unusedPattern->filePath . ':' . $unusedPattern->line], $this->getIdentifier());
        }
        return $ruleErrors;
    }
