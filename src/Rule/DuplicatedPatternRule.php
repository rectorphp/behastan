...
final class DuplicatedPatternRule implements RuleInterface
{
    /**
     * @readonly
    ...
    public function process(array $contextFiles, array $featureFiles, PatternCollection $patternCollection, string $projectDirectory): array
    {
        ...
        foreach ($groupedByPattern as $pattern => $sameMaksClassMethodContextDefinitions) {
            ...
            $ruleErrors[] = new RuleError(sprintf('Duplicated pattern "%s"', $pattern), $lineFilePaths, $this->getIdentifier());
        }
        return $ruleErrors;
    }
