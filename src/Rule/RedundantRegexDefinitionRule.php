...
use Behastan202606\Symfony\Component\Finder\SplFileInfo;
final class RedundantRegexDefinitionRule implements RuleInterface
{
    /**
    ...
    public function process(array $contextFiles, array $featureFiles, PatternCollection $patternCollection, string $projectDirectory): array
    {
        ...
        foreach ($regexPatterns as $regexPattern) {
            ...
            $errorMessage = sprintf('The regex pattern "%s" is redundant. Remove start + end and use plain string exact string instead.', $regexPattern->pattern);
            $ruleErrors[] = new RuleError($errorMessage, [$regexPattern->filePath . ':' . $regexPattern->line], $this->getIdentifier());
        }
        return $ruleErrors;
    }
